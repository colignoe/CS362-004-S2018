...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  //     }
  //     return 0;
      adventurerEffect(drawntreasure, currentPlayer, state);
  ...
  //
  //     return 0;
      ambassadorEffect(choice1, choice2, handPos, state, currentPlayer);
    case cutpurse:
...
int smithyEffect(int handPos, int currentPlayer, struct gameState *state)
{
  ...
  int i;
  // for (i = 0; i < 3; i++)
  for (i = 0; i < 4; i++) // FIXME introduced bug
  {
    drawCard(currentPlayer, state);
  }
  ...
  if (choice1 > 0)
  {
    ...
    while(card_not_discarded)
    {
      if (state->hand[currentPlayer][p] == estate)
      {
        ...
        // state->coins += 4;//Add 4 coins to the amount of coins
        state->coins = 4;//Add 4 coins to the amount of coins // FIXME introduced bug
        state->discard[currentPlayer][state->discardCount[currentPlayer]] = state->hand[currentPlayer][p];
        state->discardCount[currentPlayer]++;
        for (;p < state->handCount[currentPlayer]; p++)
        {
...
int ambassadorEffect(int choice1, int choice2, int handPos, struct gameState *state, int currentPlayer)
{
  ...
  {
    // if (i != currentPlayer)
    if (i == currentPlayer) // FIXME introduced bug
    {
      gainCard(state->hand[currentPlayer][choice1], state, 0, i);
    }
  }
